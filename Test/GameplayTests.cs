...
public class GameplayTests : TestBase
{
    ...
    {
        var location = _gameplay.GetOrGenerateLocation(GameplayLore.Locations.Dragonmaw.Soudheim.Danar.Arada.Position);

        _snapshot.Locations.Count.Should().Be(1);
        location.Position.Should().Be(GameplayLore.Locations.Dragonmaw.Soudheim.Danar.Arada.Position);
        ...
        location.Effort.Should().BeLessThanOrEqualTo(GameplayLore.Locations.Dragonmaw.Soudheim.Danar.Arada.Effort);
        location.Quests.Count.Should().BeGreaterThanOrEqualTo(1);
        location.Description.Should().NotBeNullOrWhiteSpace();
        location.FullName.Should().Be(GameplayLore.Locations.Dragonmaw.Soudheim.Danar.Arada.FullName);
